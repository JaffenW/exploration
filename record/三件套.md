...
4. 抽象语法树AST：源代码经过词法分析和语法分析后产生的一种树状结构，是对源代码的一种抽象，转化为抽象语法树有利于程序的分析和处理。
...
## 浏览器储存
1. cookie一般由服务器生成，可以设置过期时间，每次请求都会携带在header中，存储大小4k（可以设置Domain和Path让指定一级域名的某个路径可以访问cookie
2. sessionStorage是会话缓存，在页面关闭后清除，大小5m左右
3. localstorage是本地缓存，一直有效，大小5m左右
...
- Access-Control-Allow-Origin：请求域名
2. 客户端请求设置
...
1. 主要字段
	...
	- 协商缓存：Last-Modified/If-Modified-Sinse，ETag/If-Non e-Match
2. 详细
