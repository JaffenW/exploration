...
[揭秘计算机之间互发数据的关键原理](https://baijiahao.baidu.com/s?id=1628221031868284310&wfr=spider&for=pc)
## 2.状态码
1. 100：请求者应当继续请求
2. 101：切换协议
3. 200： 正常返回
...
- **1.1**：支持长连接（增加Connection字段，设置keep-alive）、支持断点续传（使用请求头的range实现,主要应用于大文件上传和下载，因网络或其他原因中断后可以从中断的位置继续进行）、使用管道传输（多个请求同时发送，服务端还是按顺序响应）
...
1. 在正常网站上插入恶意脚本，点击了后将用户信息发送到恶意脚本指定的服务器上
2. 攻击者利用网站的漏洞往web页面插入恶意的脚本，当用户访问了带有恶意脚本的网页时，就会触发这些恶意脚本，这些脚本可能会把用户的一些信息给发送到攻击者的服务器上，造成信息的泄漏。其中XSS攻击又分为反射型、存储型和dom型：
