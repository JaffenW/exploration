...
----------
1. Vue和React都是非常好的框架，他们减少了我们很多dom的操作，让我们只要专注于业务逻辑的处理，提升了我们的开发效率，在使用这两个框架的过程中，其实他们确实是有很多不一样的地方
2. 比如说我们最明显能能感受到的，Vue呢是专门弄了一个Vue文件，在这个Vue文件中html、js、css是完全分开的，结构很清晰明了，这也是我比较喜欢Vue的一个点，而React呢是用jsx的一个语法，他是将html跟我们的业务逻辑也就是js是组合在一起的，这使得它可以用三目运算呀、dom数组去渲染页面，这样的话是相对比较灵活一点，而Vue要实现这样的dom的渲染隐藏、列表的渲染需要通过它内置的一些指令
3. 而在我们一些数据的更新上，vue是采用响应式的，数据是同步更新的，也就是在我们修改了数据之后我们能在下一条语句中就能直接取到它的最新值，而react采用的是函数式的更新方式，这种更新呢数据是异步更新的，这就使得我们要经常考虑它数据异步更新的问题。而Vue这种响应式更新的方式就使得它可以进行一些比较小粒度的更新，而React呢是进行全量更新的，所以我们开发的过程中就要去考虑一些避免它不必要的更新问题
4. 所以从整体来看的话，React是比较粗狂一点的，这会给我们项目更多的一些灵活性，这也是为什么一些比较大型的项目或者大厂会比较喜欢用React，这种更大的灵活性使得他们可以基于React去做更好的开发，更丰富的一些方案。而Vue的话就做的比较细致一点，很多情况都给你考虑好了，做了更好的一些支持
5. 除此之外就是一些语法上的，其实这个也没必要说什么，基本上不同的框架它的语法也都不一样
